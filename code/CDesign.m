...
[C,CRe,theta_m,Coil_Data] = coilassign(Wnd.Qs,Wnd.p, ...
    Wnd.m,Wnd.yd,Wnd.nl,Wnd.x);
Wnd.C = C;
Wnd.CRe = CRe;
Wnd.theta_m = theta_m;
Wnd.Coil_Data = Coil_Data;
...
function [C,CRe,theta_m,Coil_Data] = coilassign(Qs,p,varargin)
% [Xsi,Coil_Data] = Coilassign(Qs,p,varargin)
% 
...
tau_s = 2*pi/Qs;
phase_belt = 2*pi/(2*m);
Coil_Names = cell(1,2*m);
n_phase_belts = 2*pi*p/phase_belt;
Np = n_phase_belts;
C   = zeros(m,Qs);
CRe = zeros(m,Qs);
Coil_Data = cell(Qs,nl);
if nl == 2; x = 1; end;

for j = 1:2*m
    jj = mod(j,m);
    if jj == 0; jj = m; end;
    % Coil_Names = {A BRe ... ARe B ...}
    if -1*cos(mod(j,2)*pi) > 0
        Coil_Names{j} = [char(64+jj)];
    else
        Coil_Names{j} = [char(64+jj) 'Re'];
    end
end
...
for n1 = 1:Np
    ...
    for n2 = lower_bound:high_bound
        ...
        if (theta_e >= theta_1) & (theta_e < theta_2) | phase_interval
            ...
            %
            C(row,n2) = (-1)^(k-1);
            Coil_Data{n2,1} = [Coil_Names{k} num2str(n2)];
            %
            ...
            kre = Return_Path(k);
            if nl == 2
                Coil_Data{PhRe,2} = [Coil_Names{kre} num2str(n2)];
            else
                Coil_Data{PhRe,1} = [Coil_Names{kre} num2str(n2)];
            end
            sumC = sum(abs(C));
            sumCRe = sum(abs(CRe)); 
        end
